...
def get_args():
    ...
    parser.add_argument('--pretrained', type=str, default='ckpt/1/model_best.pth', help="pretrained model")

    parser.add_argument('--test_data', type=str, default='data/image_path/test.txt')
...
def save_denoise_img(pred, data_path, save_img=True):
    ...
    save_path = data_path[:11] + "_new_" + data_path[11:]
...
def main():
    ...
    with torch.no_grad():
        for inp_path in tqdm(data_list):
            ...
            inp_data = inp_data.permute(1, 0, 2, 3)
            pred = model(inp_data)
            pred = pred.permute(1, 0, 2, 3).squeeze(0)
            pred[pred < 0] = 0
            pred[pred > 1] = 1
